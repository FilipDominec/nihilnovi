...
for x, y, param, label, xlabel, ylabel, color in zip(xs, ys, params, labels, xlabels, ylabels, colors):
    ...
    x1 = 2*np.pi*nGaN(1239.8/x)/(x*1e-9)
    # (1 eV photon has wavelength of 1239.8 nm in vacuum)

    ## interpolate so that the x-axis is in increasing order and equidistant
    x2 = np.linspace(np.min(x1), np.max(x1), len(x1)*100) ## finer resample to keep all spectral details
    y2 = np.interp(x2, x1[::-1], (y[::-1]))
    #x2 = x1[::-1] #np.linspace(np.min(x1), np.max(x1), len(x1)*4) ## finer resample to keep all spectral details
    #y2 = y[::-1] #np.interp(x2, x1[::-1], (y[::-1]))

    ## using FFT, x2 is transformed roughly into "thicknesses of virtual Fabry-Perot resonators"
    xf = np.fft.fftfreq(len(x2), x2[1]-x2[0])
    yf = np.fft.fft(y2)
    #print(np.min(x1), np.max(x1),x2)
    ...
    yf[findmax-5:findmax+5] = (yf[findmax-5] + yf[findmax+5])/5
...
ax.set_title('')
#ax.legend(loc='best', prop={'size':6})#!/usr/bin/python3
#-*- coding: utf-8 -*-
